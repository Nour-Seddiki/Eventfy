...
async def save_event(user: user_dependency, db: db_dependency, data: SavingEventCreate):
...
async def get_my_saved_events(user: user_dependency, db: db_dependency):
...
async def remove_saved_event(user: user_dependency, db: db_dependency, saving_event_id: int = Path(gt=0)):
...
async def check_if_saved(user: user_dependency, db: db_dependency, event_id: int = Path(gt=0)):
...
async def get_saved_count(user: user_dependency, db: db_dependency, event_id: int = Path(gt=0)):

...
async def create_review(user: user_dependency, db: db_dependency, data: ReviewRequest):
    return ReviewService().create_review(user, db, data)


@router.get("/event_reviews/{event_id}", status_code=status.HTTP_200_OK)
async def get_event_reviews(user: user_dependency, db: db_dependency, event_id: int = Path(gt=0)):
...
async def get_organizer_reviews(user: user_dependency, db: db_dependency, organizer_id: int = Path(gt=0)):
...
async def update_review(user: user_dependency, db: db_dependency, data: UpdateReview, review_id: int = Path(gt=0)):

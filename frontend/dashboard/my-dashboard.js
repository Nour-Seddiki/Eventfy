 ...
 * Fetches real user tickets from the backend API.
 */
(function () {
  'use strict';
  ...
  /** Render a list of ticket/event objects into the DOM */
  function renderEvents(events) {
    ...
    list.querySelectorAll('.btn-view-event').forEach(btn => {
      btn.addEventListener('click', () => {
        ...
        window.location.href = `../Events/index.html?eventId=${eventId}`;
      });
    });
  }

  /** Fetch user's activity from the API and render */
  async function loadDashboard() {
    ...
    try {
      ...
      // For each ticket, try to get event details
      // The activity response already has ticket data
      // Let's also fetch the public events for titles/locations
      let allEvents = [];
      try {
